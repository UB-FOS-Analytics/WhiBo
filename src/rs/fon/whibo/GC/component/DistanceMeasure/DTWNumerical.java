...
/**
 * A distance measure based on "Dynamic Time Warping". The DTW distance is
 * mapped to a similarity measure using f(x)= 1 - (x / (1 + x)). Feature weights
 * are also supported.
 */

public class DTWNumerical extends AbstractDistanceMeasure {
	/**
	 * Instantiates a new DTW numerical distance measure.
	 * 
	 * @param parameters
	 *            the parameters
	 */
	public DTWNumerical(List<SubproblemParameter> parameters) {
		super(parameters);
	}
