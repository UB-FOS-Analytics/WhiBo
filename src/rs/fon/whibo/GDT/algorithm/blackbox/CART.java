...
public class CART extends AbstractLearner implements Learner {
	...
	public List<ParameterType> getParameterTypes() {
		...
		type = new ParameterTypeInt(TRADE_ERROR_FOR_SIMPLICITY,
				"Weather to introduce error in order to get simpler trees", 0,
				1, 0);
		type.setExpert(false);
		types.add(type);
	...
	public void doWork() throws OperatorException {
		...
		int trade = getParameterAsInt(TRADE_ERROR_FOR_SIMPLICITY);
		...
		problem.getSubproblems().get(3).getMultipleStepData().get(0)
				.getListOfParameters().get(0)
				.setXenteredValue(String.valueOf(trade));
